...
const AUTH_REQUIRED_ERROR =
  "Gmail is not authorized. Ask the user for confirmation via AskUserQuestion, then call gmail_authorize to start the OAuth flow.";
...
function googleApiPost(
  ...
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(
              new Error(
                `Google API ${hostname}${path} failed (HTTP ${res.statusCode}): ${data}`,
              ),
            );
          }
        });
    ...
    req.setTimeout(GOOGLE_API_TIMEOUT_MS, () => {
      req.destroy(
        new Error(
          `Google API ${hostname}${path} timed out after ${GOOGLE_API_TIMEOUT_MS / 1000}s`,
        ),
      );
    });
    req.write(bodyBuf);
...
async function withImapClient(cfg, fn) {
  ...
  for (let attempt = 0; attempt < IMAP_RETRY_ATTEMPTS; attempt++) {
    if (attempt > 0) {
      await new Promise((r) =>
        setTimeout(r, IMAP_RETRY_DELAYS_MS[attempt - 1]),
      );
    }
    ...
    try {
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  `IMAP connect timed out after ${IMAP_CONNECT_TIMEOUT_MS}ms`,
                ),
              ),
      ...
      const isRetryable =
        /connection not available|connect timed out|ECONNRESET|ECONNREFUSED|EHOSTUNREACH|ETIMEDOUT|socket|network/i.test(
          msg,
        );
...
function toSummary(mailbox, item, bodyText, attachmentCount) {
  ...
  return {
    ...
    preview: truncate(
      compactWhitespace(bodyText) ||
        compactWhitespace(item.envelope?.subject ?? "") ||
        "",
      200,
    ),
    flags,
    ...
    hasAttachments:
      typeof attachmentCount === "number" ? attachmentCount > 0 : false,
    messageId: Array.isArray(item.envelope?.messageId)
...
function buildServerCriteria(params, client, cfg) {
  if (params.gmailRaw) {
    if (!isGmailHost(cfg.imap.host) || !clientHasGmailExt(client)) {
      throw new Error(
        "gmailRaw was provided but the connected server does not advertise X-GM-EXT-1.",
      );
    }
...
function createRuntime(cfg) {
  return {
    ...
    async searchMessages(params) {
      ...
      const fetchMode = chooseFetchMode(
        queryGroups,
        effective.hasAttachment !== undefined,
      );
      return withImapClient(cfg, (client) =>
        withMailboxLock(client, mailbox, async () => {
          const { criteria, gmailRawUsed } = buildServerCriteria(
            effective,
            client,
            cfg,
          );
          ...
          const matchedUids = (Array.isArray(matchedRaw) ? matchedRaw : []).map(
            Number,
          );
          const baseInfo = {
            ...
            pickedOperators: pickedOperators.length
              ? pickedOperators
              : undefined,
          ...
          if (!matchedUids.length) {
            return {
              messages: [],
              info: { ...baseInfo, scanned: 0, filteredClientSide: 0 },
            };
          }
          ...
          try {
            for await (const item of client.fetch(
              buildImapSequenceSet(fetchTargets),
              fetchQuery,
              { uid: true },
            )) {
          ...
          matches.sort(
            (a, b) =>
              (parseDate(b.date)?.valueOf() ?? 0) -
              (parseDate(a.date)?.valueOf() ?? 0),
          );
          return {
            ...
            info: {
              ...
              partial: timedOut
                ...
                : capped
                  ? {
                      reason: `server returned ${matchedUids.length} matches; fetched most recent ${FETCH_CANDIDATES_CAP}`,
                      processed: fetchTargets.length,
                      remaining: matchedUids.length - fetchTargets.length,
                    }
                  : undefined,
            },
          };
    ...
    async getMessage(params) {
      ...
      return withImapClient(cfg, (client) =>
        withMailboxLock(client, mailbox, async () => {
          ...
          const item = await client.fetchOne(String(params.uid), fetchQuery, {
            uid: true,
          });
    ...
    async getThread(params) {
      ...
      return withImapClient(cfg, (client) =>
        withMailboxLock(client, mailbox, async () => {
          ...
          const seed = await client.fetchOne(String(params.uid), seedQuery, {
            uid: true,
          });
          ...
          const matchedRaw =
            (await client.search({ threadId }, { uid: true })) ?? [];
          const uids = (Array.isArray(matchedRaw) ? matchedRaw : [])
            .map(Number)
            .sort((a, b) => a - b);
    ...
    async downloadAttachments(params) {
      requireAuth(cfg);
      const mailbox = params.mailbox?.trim() || cfg.defaultMailbox;
      const filterSet = params.filenames?.length
          ...
          for (const att of parsed.attachments ?? []) {
            ...
            if (filterSet && att.filename && !filterSet.has(att.filename)) {
              skipped.push({
                ...
                reason: "not in filenames filter",
              });
              continue;
            }
      ...
      return withImapClient(cfg, (client) =>
        withMailboxLock(client, mailbox, async () => {
          if (typeof params.read === "boolean") {
            const op = params.read
              ? client.messageFlagsAdd
              : client.messageFlagsRemove;
            await op.call(client, String(params.uid), ["\\Seen"], {
              uid: true,
            });
          }
          if (typeof params.flagged === "boolean") {
            const op = params.flagged
              ? client.messageFlagsAdd
              : client.messageFlagsRemove;
            await op.call(client, String(params.uid), ["\\Flagged"], {
              uid: true,
            });
          }
          return {
...
function formatMailboxList(mailboxes) {
  ...
  return mailboxes
    .map(
      (mb) =>
        `- ${mb.path}${mb.specialUse ? ` [${mb.specialUse}]` : ""}${mb.flags.length ? ` flags=${mb.flags.join(",")}` : ""}`,
    )
    .join("\n");
}
function formatMessageList(messages, info) {
  ...
  const parts = info
    ? [
        ...
        info.effectiveSince
          ? `since=${info.effectiveSince.slice(0, 10)}`
          : null,
        info.pickedOperators?.length
          ? `extracted=[${info.pickedOperators.join(" ")}]`
          : null,
        info.effectiveQuery ? `q="${info.effectiveQuery}"` : null,
        info.partial
          ? `PARTIAL: ${info.partial.reason} (${info.partial.processed}/${info.partial.processed + info.partial.remaining})`
          : null,
      ]
        .filter(Boolean)
        .join(", ")
...
function formatMessage(message) {
  ...
  lines.push(
    "",
    bodyHeader,
    truncate(message.bodyText || "(empty)", BODY_TEXT_LIMIT),
  );
  return lines.join("\n");
}
...
const gmailPlugin = {
  ...
  name: "Gmail",
  description:
    "Read, search (server-side IMAP + Gmail X-GM-RAW), send, reply, organize, and download attachments using a Gmail App Password.",
  register(api) {
    ...
    api.registerTool({
      ...
      description:
        "Start Gmail OAuth 2.0 authorization. Opens a browser for the user to grant Gmail access, waits for completion, and saves the tokens to disk. Blocks until the browser flow completes. Returns the authorized email on success.",
      parameters: Type.Object({
        forceRelogin: Type.Optional(
          Type.Boolean({
            description: "Force re-authorization even if already authorized",
          }),
        ),
      }),
      async execute(_id, params) {
        ...
        if (!params.forceRelogin && config.refreshToken && config.email) {
          return toolTextResult(
            `Gmail is already authorized as ${config.email}.`,
            { status: "ok", email: config.email, alreadyAuthorized: true },
          );
        }
        const clientId = GOOGLE_APP_CLIENT_ID || config.clientId || "";
        const clientSecret =
          GOOGLE_APP_CLIENT_SECRET || config.clientSecret || "";
        if (!clientId || !clientSecret) {
          throw new Error(
            "Missing OAuth credentials. Set GOOGLE_APP_CLIENT_ID and GOOGLE_APP_CLIENT_SECRET in plugins.config.json → environment.gmail, or as environment variables.",
          );
        }
        const port = 18080;
        ...
        console.error(
          `[gmail_authorize] Starting OAuth server on ${redirectUri}`,
        );
        return new Promise((resolvePromise, rejectPromise) => {
          ...
          const server = http.createServer(async (req, res) => {
            ...
            if (error) {
              console.error(
                `[gmail_authorize] OAuth error from Google: ${error}`,
              );
              res.writeHead(200, {
                "Content-Type": "text/html; charset=utf-8",
              });
              res.end(
                "<html><body><h3>Authorization Denied</h3><p>You may close this window.</p></body></html>",
              );
              cleanup();
              server.close();
              resolvePromise(
                toolTextResult("Authorization was denied by the user.", {
                  status: "error",
            ...
            if (!code) {
              console.error(
                "[gmail_authorize] No authorization code in callback",
              );
              res.writeHead(200, {
                "Content-Type": "text/html; charset=utf-8",
              });
              res.end(
                "<html><body><h3>Error</h3><p>No authorization code received.</p></body></html>",
              );
              cleanup();
              server.close();
              resolvePromise(
                toolTextResult("No authorization code received.", {
                  status: "error",
            ...
            try {
              const tokenResp = await googleApiPost(
              ...
              const refreshTokenVal =
                tokenResp.refresh_token || config.refreshToken || "";
              ...
              console.error(
                `[gmail_authorize] Authorization successful for ${email || "unknown"}`,
              );
              res.writeHead(200, {
                "Content-Type": "text/html; charset=utf-8",
              });
              res.end(
                "<html><body><h3>Authorization Successful!</h3><p>You may close this window and return to TelyClaw.</p></body></html>",
              );
              cleanup();
              server.close();
              resolvePromise(
                toolTextResult(
                  `Gmail authorization successful! Authorized as ${email || "unknown"}.`,
                  { status: "ok", email: email || null },
                ),
              );
            } catch (err) {
              console.error(
                `[gmail_authorize] Token exchange failed: ${err.message}`,
              );
              res.writeHead(200, {
                "Content-Type": "text/html; charset=utf-8",
              });
              res.end(
                `<html><body><h3>Error</h3><p>Token exchange failed: ${err.message}</p></body></html>`,
              );
              cleanup();
              server.close();
              resolvePromise(
                toolTextResult(`Token exchange failed: ${err.message}`, {
                  status: "error",
                  error: err.message,
                }),
              );
          ...
          server.on("error", (err) => {
            console.error(
              `[gmail_authorize] Server error: ${err.code} - ${err.message}`,
            );
            if (err.code === "EADDRINUSE")
              rejectPromise(
                new Error(
                  `Port ${port} is already in use. Is another OAuth flow or application running on that port?`,
                ),
              );
          ...
          timeoutId = setTimeout(() => {
            console.error(
              "[gmail_authorize] Timed out waiting for OAuth callback",
            );
            server.close();
            rejectPromise(
              new Error(
                `OAuth authorization timed out after ${OAUTH_SERVER_TIMEOUT_MS / 1000}s. The browser callback to ${redirectUri} was not received. Check that: (1) the Google Cloud Console has this exact redirect URI in the authorized list, (2) no firewall is blocking localhost connections.`,
              ),
            );
          ...
          server.listen(port, "0.0.0.0", () => {
            ...
            console.error(
              `[gmail_authorize] Opening browser: ${url.substring(0, 120)}...`,
            );
    ...
    api.registerTool({
      ...
      description:
        "List the mailboxes (folders/labels) on the configured account.",
      ...
      parameters: Type.Object({
        ...
        before: Type.Optional(
          Type.String({
            minLength: 1,
            description:
              "Upper bound on internalDate (ISO or YYYY-MM-DD). Use as pagination cursor.",
          }),
        ),
    ...
    api.registerTool({
      ...
      description:
        "Download all (or filtered) attachments of one message to the configured attachments directory.",
      parameters: Type.Object({
        mailbox: Type.Optional(Type.String({ minLength: 1 })),
        uid: Type.Integer({ minimum: 1 }),
      ...
      async execute(_id, params) {
        ...
        const lines = [
          ...
          ...result.saved.map(
            (a) =>
              `  - ${a.filename} (${a.contentType ?? "?"}, ${a.size}B) -> ${a.path}`,
          ),
        ];
    ...
    api.registerTool({
      ...
      label: "Update flags",
      description:
        "Mark one message as read/unread and/or set/clear its starred state.",
      parameters: Type.Object({
        mailbox: Type.Optional(Type.String({ minLength: 1 })),
        uid: Type.Integer({ minimum: 1 }),
      ...
      async execute(_id, params) {
        if (
          typeof params.read !== "boolean" &&
          typeof params.flagged !== "boolean"
        ) {
          throw new Error(
            "Provide at least one flag update: read and/or flagged.",
          );
        }
    ...
    api.registerTool({
      ...
      label: "Send message",
      description:
        "Send a new email. With requireExplicitSendConfirmation=true (default), must pass confirm=true.",
      parameters: Type.Object({
        to: recipientSchema,
    ...
    api.registerTool({
      ...
      description:
        "Reply to an existing message by UID. replyAll=true CCs all original recipients (excluding self).",
      parameters: Type.Object({
        mailbox: Type.Optional(Type.String({ minLength: 1 })),
        uid: Type.Integer({ minimum: 1 }),
        text: Type.Optional(Type.String()),
      ...
      async execute(_id, params) {
        ...
        const ownAddresses = uniqueStrings([
          lower(cfg.from),
          lower(cfg.username),
        ]);
        ...
        const references = uniqueStrings(
          [...(original.references ?? []), messageIdRef ?? ""].filter(Boolean),
        );
        let to;
        let cc = [];
        if (params.replyAll) {
